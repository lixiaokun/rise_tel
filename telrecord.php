<?php
    ...
    include"common/getid3/getid3.php";
    $getID3 = new getID3();
	session_start();
	error_reporting(0);
	$date = $_GET['date'];
	...
	$flag = 0;
    function traverse($path = '.',$action = '') {
    	global $tel,$audio_path,$flag,$getID3; 
        ...
        while(($file = readdir($current_dir)) !== false) {    //readdir()返回打开目录句柄中的一个条目
            ...
            if($file == '.' || $file == '..') {
            	...
            	if(strstr($file, $tel)){
                    if($action = 'getLy'){
                        $ThisFileInfo = $getID3->analyze($audio_path.'/'.$file);//分析文件
    ...
    foreach ($uname_record as  $value) {
        ...
        if($_GET['action'] == 'getLy'){
            traverse($file_path,$_GET['action']);
        }else{            
            traverse($file_path);
        }    	
    }    
